...
# use only predefined tags from root compartment or include all compartment tags as well
...
def DisplayInstances(instances, compartmentName, instancetype, regionname):
    for instance in instances:
        ...
        if instancetype == "DB":
            OCPU, MEM, SSD = shapes.ComputeShape(instance.shape)
            # Overwrite Shape's CPU count, with DB enabled CPU count
            OCPU = instance.cpu_core_count
            response = databaseClient.list_db_nodes(
                compartment_id=instance.compartment_id, db_system_id=instance.id)
        ...
        try:
            ...
            for customertag in customertags:
                try:
                    tagtxt = tagtxt + FieldSeperator + \
                        namespaces[customertag[0]][customertag[1]]
                except:
...
customertags = []
header = "Name,State,Service,Licensed,Version,OS,Shape,OCPU,MEMORY,SSD TB,Compartment,AD,PrivateIP,PublicIP".replace(
    ",", FieldSeperator)
...
for namespace in tags_namespaces:
    ...
    for tag in tags:
        ...
        header = header + FieldSeperator + \
            "{}.{}".format(namespace.name, tag.name)
...
for region in regions:
    ...
    response = oci.pagination.list_call_get_all_results(
        identity.list_compartments, RootCompartmentID, compartment_id_in_subtree=True)
    compartments = response.data
    ...
    for compartment in compartments:
        ...
        if compartment.lifecycle_state == "ACTIVE":
            ...
            try:
                response = oci.pagination.list_call_get_all_results(
                    ComputeClient.list_instances, compartment_id=compartmentID)

                if len(response.data) > 0:
                    DisplayInstances(response.data, compartmentName,
                                     "Compute", region.region_name)
            except:
                print("Error?")
            ...
            try:
                response = oci.pagination.list_call_get_all_results(
                    databaseClient.list_db_systems, compartment_id=compartmentID)
                if len(response.data) > 0:
                    DisplayInstances(
                        response.data, compartmentName, "DB", region.region_name)
            except:
                print("Error?")

            try:
                response = oci.pagination.list_call_get_all_results(
                    databaseClient.list_autonomous_data_warehouses, compartment_id=compartmentID)
                if len(response.data) > 0:
                    DisplayInstances(
                        response.data, compartmentName, "ADW", region.region_name)
            except:
                print("Error?")

            try:
                response = oci.pagination.list_call_get_all_results(
                    databaseClient.list_autonomous_databases, compartment_id=compartmentID)
                if len(response.data) > 0:
                    DisplayInstances(
                        response.data, compartmentName, "ATP", region.region_name)
            except:
                print("Error?")
